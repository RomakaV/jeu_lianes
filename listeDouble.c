...
typedef struct T_cell{
    struct T_cell *suiv;
    struct T_cell *prec;
    int *data;              //attention � faire un malloc sur ce champ avant de s'en servir
} *T_liste;
*/
...
T_liste getptrFirstCell(T_liste l)
{
    ...
    // bref, passer par courant est plus p�dagogique
    if (listeVide(courant)) {
        if (DEBUG) printf("Erreur getptrFirstCell liste vide");
        //DEBUG est d�fini dans le listeDouble.h
        return courant;
    }
    else {
...
int* getPtrData(T_liste l)
{
    ...
    else return l->pdata;  // ptr sur notre entier stock�
};
...
void tri_selection_liste(T_liste l)
{
    ...
    //nous ne faisons ici que des branchements et d�placements de pointeurs
    for (en_cours = l; en_cours != fin; en_cours=getptrNextCell(en_cours)) //avec "en_cours != fin", nous nous arr�tons bien sur l'avant derni�re cellule
    {
        plus_petit = en_cours;
        for (j = en_cours ; j != fin; j=getptrNextCell(j))
        {
            if (*getPtrData(j) < *getPtrData(plus_petit) )  //comparaison des contenus point�es (pas des pointeurs directement!)
